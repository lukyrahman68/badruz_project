...
class PengadaanController extends Controller
{
    ...
    public function stock(){
        $barangs = Barang::join('stocks','stocks.barang_id','=','barangs.id')
                        ...
                        ->selectRaw('*,stocks.jumlah, barangs.id as id_bar,suppliers.id as id_sup, suppliers.jenis_id as id_jenis');
                        // ->get();
                        // dd($barangs);
                        if ($barang = Barang::get()){
                            foreach ($barangs as $brg){
                                $ins = new Pengadaan;
                                $ins->barang_id = $brg->id_bar;
                                $ins->jenis_id = $brg->id_jenis;
                                $ins->supplier_id = $brg->id_sup;
